...
function multiplyArray(array) {
  ...
  for (let value of array) product *= value; // Множимо кожен елемент на загальний добуток
...
function printStrings(array) {
  ...
  result = result.repeat(2); // Повторюємо рядок 2 рази

  result = result.replace(/\s/g, ""); // Видаляємо пробіли з рядка за другим повтором

  console.log(result); // Виводимо результат на консоль
...
function concatenateStrings(array, subString) {
  ...
  if (result.includes(subString)) console.log(true);
...
function addTenToEach(array) {
  let result = []; // Ініціалізуємо масив через конструктор з такою же кількістю елементів як в вхідному масиві для зберігання результату

  for (let i in array) result.push(array[i] + 10);
...
function swapMinMax(array) {
  let max = array[0]; // Ініціалізуємо змінну `max` з першим елементом масиву
  let min = array[0]; // Ініціалізуємо змінну `min` з першим елементом масиву
  let maxIndex = 0; // Ініціалізуємо змінну `maxIndex` з нульовим значенням
  let minIndex = 0; // Ініціалізуємо змінну `minIndex` з нульовим значенням

  // Ітеруємося по масиву за допомогою циклу for
  ...
  // Використовуємо деструктуризацію для обміну місцями мінімального та максимального значень у масиві
  [array[maxIndex], array[minIndex]] = [array[minIndex], array[maxIndex]];

  // Повертаємо масив з обміненими місцями мінімального та максимального значень
  return array;
...
function getEvenNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr[i] % 2 === 0) {
      ...
      result.push(arr[i]); // Додаємо його до масиву результату
    }
  }
...
function multiplyByIndex(arr) {
  let result = []; //Створюємо порожній масив для зберігання результату

  for (
    let i = 0;
    i < arr.length;
    i++ // Проходимо через кожен елемент вхідного масиву
  )
